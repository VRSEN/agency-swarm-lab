...
class ComponentInjectorTool(BaseTool):
    ...
    file_path: str = Field(
        ..., description="The file path of the existing React component."
    )
    additional_imports: List[str] = Field(
        ...,
        description="Additional import statements for the new component. Example: ['import React from 'react']. Use empty list if no additional imports are needed.",
    )
    component: Union[ReactComponent, str] = Field(
        ..., description="The new component to inject."
    )
    target_component: str = Field(
        ...,
        description="The name of the component into which the new component should be injected.",
    )
    index: int = Field(
        ...,
        description="The index of the target component to inject into (0-based index). In case when there are multiple components of the same type, use the index to specify which one to inject into.",
    )
    insert_index: int = Field(
        ...,
        description="The index at which to inject the new component. If the target component contains children, use the insert_index to specify which one to inject into. (0-based index)",
    )
    ...
    def inject_component(
        ...
        if not isinstance(parsed_json, dict):
            print(
                f"Error: Expected parsed_json to be a dict, but got {type(parsed_json)}"
            )
            return parsed_json
        ...
        if parsed_json["type"] == target_type and index == 0:
            ...
            if "props" in parsed_json and "children" in parsed_json["props"]:
                ...
                if isinstance(children, dict):
                    parsed_json["props"]["children"] = self.inject_component(
                        children,
                        new_component,
                        target_type,
                        index - (1 if parsed_json["type"] == target_type else 0),
                    )
                elif isinstance(children, list):
                    for i, child in enumerate(children):
                        # Ensure child is a dictionary before recursive call
                        if isinstance(child, dict):
                            parsed_json["props"]["children"][i] = self.inject_component(
                                child,
                                new_component,
                                target_type,
                                index
                                - (1 if parsed_json["type"] == target_type else 0),
                            )
                        else:
                            print(
                                f"Error: Expected child to be a dict, but got {type(child)}"
                            )
                else:
                    print(
                        f"Error: Expected children to be a list or dict, but got {type(children)}"
                    )
    ...
    def parse_jsx_to_json(self, code, imports, logic):
        ...
        for imp in imports:
            if imp.startswith("import"):
                ...
                if "{" in parts[0]:
                    ...
                    commonjs_imports.append(
                        f"const {{ {names} }} = require('{module}');"
                    )
                else:
        ...
        result = subprocess.run(
            ["node", "transpiled.js"], capture_output=True, text=True
        )
    ...
    def render_json_to_jsx(
        self, component: Union[Dict[str, Any], str], indent: int = 0
    ) -> str:
        """
        ...
        props_str = " ".join(
            [f"{key}={json.dumps(value)}" for key, value in props.items()]
        )

        if children:
            if isinstance(children, list):
                children_str = "\n".join(
                    [self.render_json_to_jsx(child, indent + 2) for child in children]
                )
    ...
    def run(self):
        ...
        parsed_jsx_json = self.parse_jsx_to_json(
            code, import_lines + self.additional_imports, logic
        )
        ...
        modified_json = self.inject_component(
            parsed_jsx_json,
            self.component.custom_dump(),
