...
class Step(OpenAISchema):
    name: str = Field(
        ...,
        description="The name of the step. Do not include maintenance or placeholder steps.",
    )
    agent: Literal["Devid", "Browsing Agent"] = Field(
...
if __name__ == "__main__":
    tool = CreatePlan(
        coding_task_description="I would like you to benchmark Llama 2 on three different providers: Together, Replicate, and Perplexity. Instruct BrowsingAgent to determine their API formats and export their documentation pages as files. Then, pass these file IDs to Devid. Inform Devid that he must first read these files using the myfiles_browser tool and then write a script that sends the same prompt/parameters to all of them. API keys are stored in the following environment variables: REPLICATE_API_KEY, TOGETHER_API_KEY, PERPLEXITY_API_KEY. Please do not report back to me until the task is completed."
    )
    print(tool.run())
